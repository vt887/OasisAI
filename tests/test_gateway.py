...
# Add gateway to path
sys.path.insert(0, str(Path(__file__).parent.parent / "services" / "oasis-gateway"))
...
class TestGatewayAsk:
    def test_ask_forwards_to_agent(self, client):
        ...
        with patch.object(gateway_main._http, "post", return_value=mock_response):
            resp = client.post(
                "/ask",
                json={"question": "How does authentication work?", "repo": "myrepo"},
            )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_ask_upstream_error_returns_503(self, client):
        import httpx
...
class TestGatewayRefactor:
    def test_refactor_forwards_to_agent(self, client):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
    ...
    def test_refactor_empty_instruction_rejected(self, client):
        resp = client.post("/refactor", json={"instruction": ""})
        assert resp.status_code == 422


class TestGatewayIndex:
    def test_index_forwards_to_indexer(self, client):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "repo_name": "myrepo",
