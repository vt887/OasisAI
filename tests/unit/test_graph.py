...
sys.path.insert(
    0, str(Path(__file__).parent.parent.parent / "services" / "graph")
)
...
class TestPythonASTParser:
    ...
    def test_parse_classes(self, tmp_path: Path) -> None:
        f = tmp_path / "sample.py"
        f.write_text(_SAMPLE_PYTHON)
        module = self.parser.parse_file(str(f), repo="test")
    ...
    def test_function_calls_detected(self, tmp_path: Path) -> None:
        f = tmp_path / "sample.py"
        f.write_text(_SAMPLE_PYTHON)
        module = self.parser.parse_file(str(f), repo="test")
...
class TestGraphBuilder:
    ...
    def test_build_produces_nodes_and_edges(self, tmp_path: Path) -> None:
        f = tmp_path / "sample.py"
        f.write_text(_SAMPLE_PYTHON)
    ...
    def test_module_node_present(self, tmp_path: Path) -> None:
        f = tmp_path / "sample.py"
        f.write_text(_SAMPLE_PYTHON)
    ...
    def test_function_nodes_present(self, tmp_path: Path) -> None:
        f = tmp_path / "sample.py"
        f.write_text(_SAMPLE_PYTHON)
        graph = self.builder.build_from_files([str(f)], repo="r")
    ...
    def test_class_nodes_present(self, tmp_path: Path) -> None:
        f = tmp_path / "sample.py"
        f.write_text(_SAMPLE_PYTHON)
        graph = self.builder.build_from_files([str(f)], repo="r")
    ...
    def test_inherits_edge_present(self, tmp_path: Path) -> None:
        f = tmp_path / "sample.py"
        f.write_text(_SAMPLE_PYTHON)
        graph = self.builder.build_from_files([str(f)], repo="r")
