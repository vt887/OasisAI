...
class GraphBuilder:
    """Build a :class:`Graph` from one or more :class:`ParsedModule` objects."""
    ...
    def _add_module(self, graph: Graph, module: ParsedModule) -> None:
        ...
        for imp in module.imports:
            ...
            graph.nodes.append(
                Node(id=imp_id, kind="import", name=imp, file_path=module.file_path, repo=module.repo)
            )
            graph.edges.append(Edge(source=module_id, target=imp_id, relation="imports"))

        # Functions
        for func in module.functions:
            func_id = _make_id("function", module.repo, module.file_path, func.name)
            graph.nodes.append(
                Node(
                    id=func_id,
                    kind="function",
            ...
            graph.edges.append(Edge(source=module_id, target=func_id, relation="defines"))

            for callee in func.calls:
                callee_id = _make_id("function", module.repo, module.file_path, callee)
                graph.edges.append(Edge(source=func_id, target=callee_id, relation="calls"))
        ...
        for cls in module.classes:
            ...
            graph.edges.append(Edge(source=module_id, target=cls_id, relation="defines"))

            for base in cls.bases:
                base_id = _make_id("class", module.repo, module.file_path, base)
                graph.edges.append(Edge(source=cls_id, target=base_id, relation="inherits"))
