...
Provides a small async utility to test whether a TCP port on a
host is reachable.

This module is intentionally small and dependency-free so it can be
used from health checks and test scripts.
"""
...
async def is_port_open(host: str, port: int) -> bool:
    ...
    try:
        ...
        # Resolve host (uses socket under the hood) before trying to
        # open a connection.
